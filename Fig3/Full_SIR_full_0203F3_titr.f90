...
!! Numerical simulation of a stochastic chemical reaction network using Gillespie's algorithm.
	...
	real(kind=dp), dimension(5) :: c_vect
...
!							15) background deacetylation of methyl & acetyl
  ...
  write(*,*) csir
	...
	KKe=1.0!1.5 but 1.0 for V5 tag
	KKi=1.0
	KKn=0.4
	...
	write (unit=9,fmt=*) csir, (Xcumule(3)/float(tgen)+Xcumuls(3)/float(tgen))/wtNormBulk, &
			(Xcumule(5)/float(tgen)+Xcumuls(5)/float(tgen))/wtNormE
  write (*,*) csir, (Xcumule(3)/float(tgen)+Xcumuls(3)/float(tgen))/wtNormBulk, &
			(Xcumule(5)/float(tgen)+Xcumuls(5)/float(tgen))/wtNormE
