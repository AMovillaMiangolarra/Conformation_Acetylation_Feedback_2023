...
!! Numerical simulation of a stochastic chemical reaction network using Gillespie's algorithm.
	...
	integer :: tot_time, t1, k, SIRcountEND, i, j, concl, trans_sil, muts
	...
	real(kind=dp) :: interac, vol_max, prob_inh, wtNormE, wtNormBulk
...
!							15) background deacetylation of methyl & acetyl
	...
	open (unit=9,status="old", file="ChIP_muts.dat", action="write")
  ...
  write(*,*) csir
	...
	ksir2=2.1!2.6, 1.8 and 6 kd , 14 kd and 8 norm bin
	kdot1=0.05 ! this is 0901
	KKd=0.35!0.52 wt and 0.3 for myc
	KKe=1.5
	KKi=1.0
	KKn=0.4
	if (muts==2) then
		KKn=0.225 !H3K79L
		kdot1=0.0 !No methylation possible in an already methyl mimic
	elseif (muts==3) then
		KKn=0.23 !sir3bah
	elseif (muts==4) then
		ksir2=0.0
	...
	if (muts==1) then
		wtNormE=Xcumule(5)/float(tgen)+Xcumuls(5)/float(tgen)
		wtNormBulk=Xcumule(3)/float(tgen)+Xcumuls(3)/float(tgen)
	endif
	...
	write (unit=9,fmt=*) muts, (Xcumule(3)/float(tgen)+Xcumuls(3)/float(tgen))/wtNormBulk, &
			(Xcumule(5)/float(tgen)+Xcumuls(5)/float(tgen))/wtNormE
  write (*,*) muts, (Xcumule(3)/float(tgen)+Xcumuls(3)/float(tgen))/wtNormBulk, &
			(Xcumule(5)/float(tgen)+Xcumuls(5)/float(tgen))/wtNormE
	write (*,*) muts, (Xcumule(3)/float(tgen)+Xcumuls(3)/float(tgen)), &
					(Xcumule(5)/float(tgen)+Xcumuls(5)/float(tgen))
...
enddo
	...
	open(unit=8, file="wtNorm.dat", status="old", action="write")
	write(8,*) wtNormBulk, wtNormE
	close(unit=8)
