...
!! Numerical simulation of a stochastic chemical reaction network using Gillespie's algorithm.
	...
	real(kind=dp) :: interac, vol_max, prob_inh, wtNormBulk, wtNormE
...
!							15) background deacetylation of methyl & acetyl
	...
	allocate (X(Nsp), Xnew(Nsp), Xbef(Nsp), Xaft(Nsp), Xrest(2), Xrep(2))
	...
	open (unit=9,status="old", file="ChIP_muts.dat", position="append", action="write")
  ...
  write(*,*) csir
	...
	do k=1,L+1
		...
		vol_locus(k)=2.0 !H4K16Q no dynamic vol.
	enddo
	!write(*,*) vol_locus(:)

	profile(:,:)=0

	p(1,:)=(/0,0,0,-1,0,1/)	!dot1 meth
	p(2,:)=(/1,0,0,0,0,0/)  !ei
	p(3,:)=(/-1,0,0,0,0,0/)
	p(4,:)=(/0,1,0,-1,0,0/) !en
	p(5,:)=(/0,-1,0,1,0,0/)
	p(6,:)=(/0,0,1,-1,0,0/) !in
	p(7,:)=(/0,0,-1,1,0,0/)
	p(8,:)=(/0,0,0,-2,2,0/)! dimer binding
	p(9,:)=(/0,0,0,2,-2,0/)! dimer unbinding
		...
		do k=1,50
			...
			do while (time .lt. tk)
				...
				if (X(6)==distSil) then
					...
					!endif
					a(1)=kdot1*X(4)
					a(2)=kei*(1-X(1)-X(2))*(1-X(3))*24/(L*vol_locus(X(4)+X(5)+X(3)+X(2)+1))
					a(3)=X(1)/tauS
					a(4)=ken*X(4)*(1-X(1)-X(2))*24/(L*vol_locus(X(4)+X(5)+X(3)+X(2)+1))
					a(5)=X(2)/tauS
					a(6)=kin*X(4)*(1-X(1)-X(3))*24/(L*vol_locus(X(4)+X(5)+X(3)+X(2)+1))
					a(7)=X(3)/tauS
					a(8)=kk*X(4)*(X(4)-1)*24/(L*vol_locus(X(4)+X(5)+X(3)+X(2)+1)*2.0)
					a(9)=X(5)/(2.0*tauS)
					A0=sum(a)
					mu=r2*A0
					do j=1,Nreact
			...
			profile(X(6)+1,1)=profile(X(6)+1,1)+1 !Methyl
			profile(X(1)*2+X(2)+X(3)+X(5)+1,2)= &
																profile(X(1)*2+X(2)+X(3)+X(5)+1,2)+1
		enddo
		if ((X(2)+X(3)+X(4)+X(5)).lt.distSil/2.0) then
			silence(i)=1
			Xcumule(:)=Xcumule(:)+(/1.0_dp*(X(6)),1.0_dp*(X(6)),1.0_dp*(X(5)+X(2)+X(3)),1.0_dp*(X(4)),&
																				1.0_dp*(X(1)+X(2)),1.0_dp*(X(1)+X(3))/)
		else
			!write (unit=9, fmt=*) i, 0
			silence(i)=0
			Xcumuls(:)=Xcumuls(:)+(/1.0_dp*(X(6)),1.0_dp*(X(6)),1.0_dp*(X(5)+X(2)+X(3)),1.0_dp*(X(4)), &
																	1.0_dp*(X(1)+X(2)),1.0_dp*(X(1)+X(3))/)
			sil_dist(X(2)+X(3)+X(4)+X(5)+1)=sil_dist(X(2)+X(3)+X(4)+X(5)+1)+1.0
		endif
		Xnew(:)=X(:)
		Xrest=(/X(2)+X(3)+X(4)+X(5),X(6)/)
		Xrep=(/X(2)+X(3)+X(4)+X(5),X(6)/)
		prob_inh=0.5
		Xbef=X
		do k=1, L/2
		call random_number(r1)
	...
	open(unit=8, file="wtNorm.dat", status="old", action="read")
	read(8,*) wtNormBulk, wtNormE
	close(unit=8)
	write(*,*) wtNormBulk, wtNormE
	!write (unit=8, fmt=*) csir, sum(silence(:))/float(tgen), sum(transit(:))/float(2*tgen), &
	 			!	sum(ver_sil(:))/float(tgen), sum(ver_trans(:))/float(2*tgen)

	write (9,*) "H4K16Q", (Xcumule(3)/float(tgen)+Xcumuls(3)/float(tgen))/wtNormBulk, &
