...
!! Numerical simulation of a stochastic chemical reaction network using Gillespie's algorithm.
	...
	real, dimension(7) ::concs
	real(kind=dp), dimension(:), allocatable :: a
	real(kind=dp) :: r1, r2, rn, ti, tk, mu, A0
	real(kind=dp), dimension(:), allocatable :: Xcumule, Xcumuls, trans_bef, trans_after, sil_dist, weights, exp_dist
...
!							15) background deacetylation of methyl & acetyl
	...
	allocate (Xcumule(6),Xcumuls(6), trans_bef(L+1), trans_after(L+1), sil_dist(L+1), exp_dist(L+1), weights(L+1))
	...
	!(concl)**2*0.05**2 !
	concs=(/0.08,0.095,0.11,0.125, 0.14, 0.155, 0.17/)

	do concl= 1,size(concs)
	csir=concs(concl)
  write(*,*) csir
	...
	KKd=0.5!0.52 wt and 0.3 for myc/V5 tag
	KKe=1.5!2.0 but 1.0 for V5 tag
	KKi=1.0
	KKn=0.4
	...
	!open (unit=9,status="old", file="silence.dat", action="write")
	...
	outer: do i=1,tgen
		...
		if ((X(2)+X(3)+X(4)+X(5)).lt.distSil/2.0) then
			...
			exp_dist(X(6)+X(8)+1)=exp_dist(X(6)+X(8)+1)+1.0
	...
	enddo
					...
					do i=1,distSil+1
						weights(i)=trans_after_e(i)*(i-1)/float(trans_sil)
					enddo
					max_after=nint(sum(weights))
					write(*,*) max_after
					switch_est=0.0
					do i=1,distSil+1
						if ((i-1).lt.max_after) then
							bin_cum=1.0
						else
							bin_cum=0.0
							do j=0,max_after-1
						...
						switch_est=switch_est+exp_dist(i)/float(sum(silence(:)))*bin_cum
						!write(*,*) bin_cum, exp_dist(i)/float(sum(silence(:))), switch_est
					enddo
				stats(7)=switch_est
				!Mean of unm nucl after division
				do i=1,distSil+1
					weights(i)=trans_after(i)*(i-1)/float(trans_sil)
				enddo
				max_after=nint(sum(weights))
				write(*,*) max_after
				switch_est=0.0
				do i=1,distSil+1
					...
					!write(*,*) bin_cum, sil_dist(i)/float(tgen-sum(silence(:))), switch_est
				enddo
	...
	write (unit=8, fmt=*) csir, stats(1:8)
