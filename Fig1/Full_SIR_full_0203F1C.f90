...
!! Numerical simulation of a stochastic chemical reaction network using Gillespie's algorithm.
	...
	integer :: tot_time, t1, k, SIRcountEND, i, j, concl, trans_sil
	...
	Nsp=8
	Nreact=15
	distSil=24 !In tetramers, not octamers.
	L=distSil
...
!							15) background deacetylation of methyl & acetyl
	...
	allocate (Xcumule(6),Xcumuls(6), trans_bef(L+1), trans_after(L+1), sil_dist(L+1), weights(L+1))
	...
	allocate(profile(distSil+3,2)) !Acetylation and ceiling of Sir proteins.

	allocate(vol_locus(L+1))

	!open (unit=8,status="old", file="silence_wt.dat", action="write")

	!Concentration units: micro-molars uM

	!csir=0.25 !do for 2,20 0.5/2.0  !Sir 3 is present in dimers, hence the factor of 2.
	!(concl)**2*0.05**2 !
	!do concl= 1,13
	csir=0.25!0.35-0.025*(concl-1)
  write(*,*) csir

	!Dissociation constants in micromolars^(-1)
	!KKe=20.0 !5 for wt, 10 for rap1, 20 for sir1. For the E silencer with V5 tag, then 10 too
	!KKi=20.0 !10 for wt, 12 for rap1, 20 for sir1
	!KKn=20.0
	!ke=csir/KKe
	!kin=csir/KKi
	!kn=csir/KKn !20 in 1611

	!KKd=1/102.5

	KKd=0.35!0.52 wt and 0.3 for myc/V5 tag
	KKe=0.6!2.0 but 1.0 for V5 tag
	KKi=0.6
	KKn=0.4
	...
	open (unit=9,status="old", file="ChIP_sim.dat", action="write")
	...
	!write (unit=8, fmt=*) csir, sum(silence(:))/float(tgen), sum(transit(:))/float(2*tgen), &
	 			!	sum(ver_sil(:))/float(tgen), sum(ver_trans(:))/float(2*tgen)
	...
	write (*,*) trans_sil
	!write (*,*) trans_bef(:)/float(trans_sil)
	!write (*,*) trans_after(:)/float(trans_sil)
	!write (*,*) Xcumule(1:2)/float(distSil*sum(silence(:)))
	!write (*,*) Xcumuls(1:2)/float(distSil*(tgen-sum(silence(:))))
	...
	write (unit=9,fmt='(*(G0.7,:,","))') "#coord"," ","Sir-e"," ","Sir-s"," ","acet-e"," ","acet-s"," ","meth-e"," ","meth-s"
	write (unit=9,fmt='(*(G0.7,:,","))') 0.05, Xcumule(5)/float(sum(silence(:))), Xcumuls(5)/float(tgen-sum(silence(:)))
	do i=1,8
		write (unit=9,fmt='(*(G0.7,:,","))') 0.05+i*0.1, Xcumule(3)/float(24*sum(silence(:))), &
			Xcumuls(3)/float(24*(tgen-sum(silence(:)))), &
			Xcumule(1)/float(24*sum(silence(:))), Xcumuls(1)/float(24*(tgen-sum(silence(:)))), &
			Xcumule(2)/float(24*sum(silence(:))), Xcumuls(2)/float(24*(tgen-sum(silence(:))))
	enddo
	write (unit=9,fmt='(*(G0.7,:,","))') 0.95, Xcumule(6)/float(sum(silence(:))), Xcumuls(6)/float(tgen-sum(silence(:)))
	close(unit=9)

	!Mean of unm nucl after division
	do i=1,distSil+1
		weights(i)=trans_after(i)*i/float(trans_sil)
	enddo
	max_after=nint(sum(weights))
	write(*,*) max_after
	switch_est=0.0
	do i=0,distSil+1
		if (i.lt.max_after) then
			bin_cum=1.0
		else
			bin_cum=0.0
			do j=0,max_after-1
				bin_cum=bin_cum+0.5**(i-1)*factorial(i-1)/(factorial(j)*factorial(i-1-j))
			enddo
		endif
		switch_est=switch_est+sil_dist(i)/float(tgen-sum(silence(:)))*bin_cum
	enddo

	!do i=1,distSil+1
	!	write (unit=11, fmt=*) i-1, sil_dist(i)/float(tgen-sum(silence(:))), trans_bef(i)/float(trans_sil), &
	!	 		trans_after(i)/float(trans_sil), trans_bef_e(i)/float(trans_sil), trans_after_e(i)/float(trans_sil)
	!enddo
	!close(unit=11)
	write(*,*) switch_est
...
!enddo
	...
	deallocate(profile) !Acetylation and ceiling of Sir proteins.
