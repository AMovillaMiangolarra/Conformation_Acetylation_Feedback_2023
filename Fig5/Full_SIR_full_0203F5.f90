...
!! Numerical simulation of a stochastic chemical reaction network using Gillespie's algorithm.
	...
	character(len=4), dimension(3) :: name
	character(len=3) :: concstr
...
!							15) background deacetylation of methyl & acetyl
  ...
  write(*,*) csir, muts
	...
	KKd=0.5!0.52 wt and 0.3 for myc
	KKe=1.5
	KKi=1.0
	if (muts==2) then
		KKe=1.0
	...
	write (concstr, '(I3.3)') nint(concs(concl)*1000)
	filename="profiles_f_"//name(muts)//"_sir4t_"//concstr//".dat"

	open (unit=10,status="new", file=filename, action="write")
	...
	do i=1,distSil+1
		write (unit=10, fmt=*) (i-1)/float(L), profile(i,1), profile(i,2)
	enddo
	close(unit=10)
